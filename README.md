...
You must first build a cross toolchain that allows DragonWare to run in 32 bit machines. A good article that explains in detail how to build `gcc` to compile operating systems can be found [here](https://wiki.osdev.org/GCC_Cross-Compiler). You must build both binutils and GCC with `i686-elf` as the `$TARGET`.
